...
mod_post = Blueprint('post', __name__)

@mod_post.route("/transactions/add/", methods=["POST"])
def add_trans():
...
@mod_post.route("/transactions/edit/<int:transaction_id>", methods=["PUT"])
...
def parseRequest(request, edit=None):
    ...
    trans['date_issued'] = dt.datetime.fromisoformat(trans.pop('datetime'))
...
@mod_post.route("/transfers/add/<int:src_id>-<int:dst_id>/", methods=["POST"])
