...
const useBalanceLineData = (props: NivoRequest) =>
    useQuery<LineData[], AxiosError>({
        queryKey: ["categories", "changes", "balanceline", props],
        queryFn: () => getAxiosData(`/api/nivo/balanceline?${searchParams(props)}`)
    });

export const BalanceLine = ({ request, size }: NivoComponentProps) => {
    const nivo = useNivoTheme();
    const theme = useMantineTheme();
    const query = useBalanceLineData(request);
